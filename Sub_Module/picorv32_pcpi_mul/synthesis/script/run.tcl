###############################################################################################################
# Raven SoC Project : PICORV32_PCPI_MUL
# Script: run.tcl
###############################################################################################################
...
create_mw_lib -technology $TECH_FILE -mw_reference_library $REFERENCE_LIBRARY picorv32_pcpi_mul.mw -open

################################################################################################################
## Read/Convert to WV GEtech/Connet Sub block to TOP block
################################################################################################################

analyze -format verilog ../../../../rtl/picorv32_pcpi_mul.v
elaborate picorv32_pcpi_mul
link
current_design
...
get_ports *clk*
read_sdc ../script/picorv32_pcpi_mul.sdc
...
write_icc2_files -output ../output/picorv32_pcpi_mul_allfiles.v
write_file -format verilog -hierarchy -output ../output/picorv32_pcpi_mul_Netlist.v

write -hierarchy -format ddc -output ../output/picorv32_pcpi_mul_hierarchy.ddc
write -hierarchy -format verilog -output ../output/picorv32_pcpi_mul_hierarchy.v

write_sdc ../output/picorv32_pcpi_mul.sdc
write_parasitics -output ../output/picorv32_pcpi_mul_parasitics
write_sdf ../output/picorv32_pcpi_mul.sdf
