...
create_mw_lib -technology $TECH_FILE -mw_reference_library $REFERENCE_LIBRARY simpleuart.mw -open

################################################################################################################
## Read/Convert to WV GEtech/Connet Sub block to TOP block
################################################################################################################

analyze -format verilog ../../../../rtl/simpleuart.v
elaborate simpleuart
link
current_design
...
get_ports *clk*
read_sdc ../script/simpleuart.sdc
...
write_icc2_files -output ../output/simpleuart_allfiles.v
write_file -format verilog -hierarchy -output ../output/simpleuart_Netlist.v

write -hierarchy -format ddc -output ../output/simpleuart_hierarchy.ddc
write -hierarchy -format verilog -output ../output/simpleuart_hierarchy.v

write_sdc ../output/simpleuart.sdc
write_parasitics -output ../output/simpleuart_parasitics
write_sdf ../output/simpleuart.sdf
